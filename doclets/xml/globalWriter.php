...
class GlobalWriter extends HTMLWriter {
	...
	function globalWriter(&$doclet) {
		...
		foreach($packages as $packageName => $package) {
			...
			$dom_globals = $doc->createElement('globals');
			...
			if ($globals) {				
				...
				foreach($globals as $global) {
					...
					$dom_global = $doc->createElement('global');
					...
					if ($textTag) {
						...
						$dom_global->appendChild($dom_description);
					}
			...
			$this->_write($package->asPath().'/package-globals.xml', 'Globals', TRUE);
		}
